...
async def upload_image_to_s3(image: BytesIO, object_name: str) -> str | None:
    session = aioboto3.Session(config_parameters.S3_ACCESS_KEY, config_parameters.S3_SECRET_KEY)
    async with session.client("s3", endpoint_url=config_parameters.S3_ENDPOINT) as s3:
        try:
            # make file public
            await s3.upload_fileobj(image, config_parameters.S3_BUCKET_NAME, object_name, ExtraArgs={"ACL": "public-read"})
...
async def save_image(image: UploadFile, image_id: int) -> str:
    ...
    try:
        ...
        image_url = await upload_image_to_s3(image.file, f"{image_id}.{image_extension}")
